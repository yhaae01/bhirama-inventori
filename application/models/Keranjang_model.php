...
class Keranjang_model extends CI_Model
{
    ...
    function json_retur_barang()
    {
        ...
        $this->datatables->join('warna w', 'dp.id_warna = w.id_warna');
        $this->datatables->join('ukuran u', 'dp.id_ukuran = u.id_ukuran');
        $this->datatables->where('jenis', 'retur_barang');
        // action
        $this->datatables->add_column(
            'action',
            '<div class="btn-group">' .
                form_open('transaksi/DetailReturBarang/deleteKeranjang') .
                form_button(['type' => 'submit', 'data-id' => '$1', 'title' => 'Hapus', 'class' => 'btn btn-danger hapusDetailReturBarang'], '<i class="fas fa-trash-alt"> </i>') .
                form_close() . '</div>',
            'id'
        );
        return $this->datatables->generate();
    ...
    // get produk dengan id_detail_produk, id_pengguna dan harga yg sama
    function get_same_varian_retur($idDetailProduk, $idPengguna, $jenis)
    {
        return $this->db
            ->where("id_detail_produk", $idDetailProduk)
            ->where("id_pengguna", $idPengguna)
    ...
    function get_id_keranjang_retur($id_detail_produk, $id_pengguna, $jenis)
    {
        return $this->db
            ->select('id')
            ->where('id_detail_produk', $id_detail_produk)
    ...
    // insert data calon pengembalian
    function insert_retur_barang($data)
    {
        $id_detail_produk = $data['id_detail_produk'];
        $id_pengguna      = $data['id_pengguna'];
        $qty              = $data['qty'];
        ...
        // start transaction
        $this->db->trans_start();
        if ($this->get_same_varian_retur($id_detail_produk, $id_pengguna, $jenis) > 0) {

            // ambil id dari tabel keranjang
            $id = $this->get_id_keranjang_retur($id_detail_produk, $id_pengguna, $jenis);
            // tambah qty dan tambah harga
            $this->db
                ->set('qty', "qty+$qty", FALSE)
                ->where($this->id, $id)
    ...
    // cek empty berdasarkan id_pengguna dan jenis
    function isEmpty($id_pengguna, $jenis)
    {
        $rows =  $this->db
            ->where('jenis', $jenis)
            ->where('id_pengguna', $id_pengguna)
